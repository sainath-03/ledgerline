...
// Senders and keywords that real transaction emails from Indian UPI
// apps and banks reliably use. Deliberately NOT scoped to subject: --
// bank alert subjects (e.g. HDFC's "You have done a UPI txn.") often
// say nothing useful; the real wording ("debited", "Paid to ...") is
// in the body, which a bare keyword searches too. Broad on purpose --
// a false positive just becomes a suggestion the user dismisses; a
// false negative means a transaction never gets suggested at all.
const GMAIL_QUERY =
  '(from:alerts@hdfcbank.bank.in OR from:googlepay-noreply@google.com OR from:noreply@phonepe.com OR from:paytmupi@paytm.com OR from:alerts@paytm.com OR debited OR "payment successful" OR "you paid" OR "UPI txn" OR "transaction alert" OR "spent on")';
...
const CATEGORY_HINTS = [
  { cat: "food", words: /swiggy|zomato|restaurant|cafe|food|eatery|dominos|starbucks|dhaba/i },
  { cat: "transport", words: /uber|ola|rapido|irctc|redbus|metro|fuel|petrol|diesel|fastag/i },
...
// Real bank UPI alerts (HDFC and similar) read like:
//   "...debited from your account ending 0067 towards VPA
//    paytm.s1qewgx@pty (MANDULA MAHESH KUMAR) on 06-09-26."
//   "...debited from your RuPay Credit Card (ending 0090)
//    Paid to Q618627174@ybl Date: 06-09-26"
// Prefer the human name in parentheses when present (most readable),
// then fall back to the raw VPA/UPI handle, then generic phrasing.
function guessMerchant(text) {
  const namePattern = text.match(/towards VPA\s+\S+\s*\(([^)]{2,60})\)/i);
  if (namePattern) return namePattern[1].trim();

  const patterns = [
    /paid to ([^\s]{2,60})/i,
    /towards VPA\s+([^\s]{2,60})/i,
...
function parseMessage(message) {
  ...
  return {
    amount,
    category,
    note: merchant || subject.replace(/[^\x20-\x7e]/g, "").trim().slice(0, 60),
    date,
